...
public enum ItoError: Error, Equatable {
    ...
    public var errorDescription: String? {
        ...
        case .memoryOutOfBounds(let offset, let length, let memorySize):
            return "Wasm memory access out of bounds at offset \(offset) with length \(length) (Memory size: \(memorySize))"
    ...
    public static func == (lhs: ItoError, rhs: ItoError) -> Bool {
        switch (lhs, rhs) {
        case let (.wasmTrap(l), .wasmTrap(r)): return l == r
        case (.memoryOutOfBounds(let l1, let l2, let l3), .memoryOutOfBounds(let r1, let r2, let r3)): 
